        ...
        def wrapper(*args, **kwargs):
            ...
            while retries < max_retries:
                try:
                    ...
                    logger.warning(f"Erro na tentativa {retries}/{max_retries}: {e}. Tentando novamente em {wait}s...")
                    time.sleep(wait)
...
def transform_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Converter datas
    df_transformed['dt_competencia'] = pd.to_datetime(df_transformed['dt_competencia']).dt.date
    df_transformed['dt_atualizacao'] = pd.to_datetime(df_transformed['dt_atualizacao']).dt.date
    ...
    df_transformed = df_transformed.drop_duplicates(subset=['estado', 'dt_competencia'], keep='last')
    ...
    if df_transformed['estado'].isnull().any() or df_transformed['dt_competencia'].isnull().any():
...
def load_to_rds(df: pd.DataFrame):
    ...
    db_url = f"postgresql+psycopg2://{DB_USER}:{quote_plus(DB_PASSWORD)}@{DB_HOST}:5432/{DB_NAME}?sslmode=require"
