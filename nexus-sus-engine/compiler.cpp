...
int main() {
    ...
    while (scanf("%s", command) != EOF) {
        if (strcmp(command, "L") == 0) {
            if (scanf("%s %s %lf %lf %lf %s %s", uf, regiao, &v_uf, &v_reg, &v_br, comp, atual) == 7) {
            ...
            if (scanf("%s", uf) == 1) {
                ...
                if (res != NULL) {
                    printf("{\"status\": \"success\", \"data\": {\"estado\": \"%s\", \"regiao\": \"%s\", \"vl_uf\": %.2f, \"vl_regiao\": %.2f, \"vl_brasil\": %.2f, \"dt_competencia\": \"%s\", \"dt_atualizacao\": \"%s\"}}\n",
                           res->estado, res->regiao, res->vl_uf, res->vl_regiao, res->vl_brasil, res->dt_competencia, res->dt_atualizacao);
                } else {
